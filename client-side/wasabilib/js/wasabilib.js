...
DataExtractor.prototype.setTarget = function(target) {
  ...
  _self.element =
    typeof target === "object"
      ? target.closest(".wasabi-ajax-element")
      : $("#" + target.closest(".wasabi-ajax-element"));
};
...
DataExtractor.prototype.getTransmissionCapableContent = function() {
  var _self = this;
  return _self.element.attr("data-json")
    ? JSON.parse(_self.element.attr("data-json"))
    : null;
};
...
DataExtractor.prototype.getUri = function() {
  ...
  return _self.element.attr("href")
    ? _self.element.attr("href")
    : _self.element.attr("data-href")
    ? _self.element.attr("data-href")
    : null;
};
 ...
 */
ResponseHandler.prototype.setExecuteRemoteProcedureCallManager = function(
  executeRemoteProcedureCallManager
) {
  var _self = this;
 ...
 */
Wasabi.prototype.setExecuteRemoteProcedureCallManager = function(
  executeRemoteProcedureCallManager
) {
  var _self = this;
...
Wasabi.prototype.registerEventHandler = function() {
  ...
  $(".wasabi-ajax-element").each(function(index) {
    ...
    if (
      !_self.registeredElements[elementId] &&
      (element.attr("data-cb") == undefined || callback != undefined)
    ) {
      ...
      if (myEvent && callback) {
        if (!_self.possibleEvents[myEvent]) {
          ...
          $(document).on(myEvent, "#" + elementId, function(event) {
            ...
            if (
              callback.dataExtractor.getElement().prop("tagName") === "FORM" &&
              callback.dataExtractor.getElement().attr("method") != "get"
            ) {
              conf["type"] = "POST";
            }
            if (callback.getCondition().check(event)) {
              var timeOut = callback.dataExtractor
                .getElement()
                .attr("data-timeout");
              timeOut != undefined
                ? callback.getCondition().setTimeout(timeOut)
                : null;
              var disableTimeout = callback.dataExtractor
                .getElement()
                .attr("data-disabletime");

              if (disableTimeout) {
                if (
                  callback.dataExtractor.getElement().prop("tagName") === "A"
                ) {
                  callback.dataExtractor
                    .getElement()
                    .attr(
                      "data-href",
                      callback.dataExtractor.getElement().attr("href")
                    )
                    .removeAttr("href");
                }
                if (
                ...
                if (disableTimeout !== "true" && $.isNumeric(disableTimeout)) {
                  ...
                  cond.checkTimeOut(function() {
                    if (
                      callback.dataExtractor.getElement().prop("tagName") ===
                      "A"
                    ) {
                      callback.dataExtractor
                        .getElement()
                        .attr(
                          "href",
                          callback.dataExtractor.getElement().attr("data-href")
                        )
...
Wasabi.prototype.send = function(url, content, config) {
  ...
  var dataJson = element.attr("data-json")
    ? JSON.parse(element.attr("data-json"))
    : {};
  ...
  _self.ajax(
    ...
    function(err) {
      ...
      if (
        element.attr("data-disabletime") != undefined &&
        element.attr("data-disabletime") == "true"
      ) {
        if (element.prop("tagName") === "A") {
          element
            .attr("href", element.attr("data-href"))
            .removeAttr("data-href");
        }
        if (element.prop("tagName") === "FORM") {
...
Wasabi.prototype.ajax = function(config, callback) {
  ...
  var jQueryConfig = {
    ...
    beforeSend: function(xhr) {
      ...
      //we add some headers
      xhr.setRequestHeader("Wasabilib", true);
      xhr.setRequestHeader(
        "Wasabilib-Request-Id",
        "id-" +
          Math.random()
            .toString(36)
            .substr(4, 32)
  ...
  $.ajax(jQueryConfig).done(function(serverResponse) {
...
NotificationCenter.prototype.register = function(observer) {
  ...
  observer.setExecuteRemoteProcedureCallManager(
    _self.wasabi.getExecuteRemoteProcedureCallManager()
  );
...
NotificationCenter.prototype.notify = function(notification) {
  ...
  jQuery.each(_self.observers, function(obs) {
    ...
    jQuery.each(notification, function(index, response) {
      if (
        window[response.responseHandler] != undefined &&
        obs instanceof window[response.responseHandler]
      ) {
        obs.update(response);
      }
    });
 ...
 */
ExecuteRemoteProcedureCallManager.prototype.execute = function(
  selector,
  methodName,
  params
) {
  ...
  if (!_self.possibleMethods[methodName]) {
    console.log(
      "The given method " + methodName + " is not a valid method name"
    );
  } else if (_self.keywords[selector]) {
    _self.keywords[selector][_self.possibleMethods[methodName]].apply(
      _self.keywords[selector],
      params
    );
 ...
 */
ExecuteRemoteProcedureCallManager.prototype.setPossibleMethods = function(
  possibleMethods
) {
  var _self = this;
...
TriggerEventManager.prototype.update = function(response) {
  ...
  if (!_self.possibleEvents[message.params[0]]) {
    console.log(
      "The given event " + message.params[0] + " is not a valid event name"
    );
  } else {
    _self.execRPCManager.execute(message.selector, message.actionType, [
      _self.possibleEvents[message.params[0]]
    ]);
  }
};
...
External_Form.prototype.setTarget = function(target) {
  var _self = this;
  _self.element =
    target.tagName === "FORM" ? target : $("#" + target.attr("form"));
  _self.jsonParams = $(target).attr("data-json")
    ? JSON.parse("[" + $(target).attr("data-json") + "]")
    : null;
};
...
$(document).ready(function() {
   ...
   */
  wasabi.setExecuteRemoteProcedureCallManager(
    new ExecuteRemoteProcedureCallManager()
  );
